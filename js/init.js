     ...
     /* <div  class="card" onclick="${movie[i].id}" class="card" "${movie[i].title}">
      ...
      function showMoviesList(array){
        ...
        for(e = 0; e < array.length; e++){
          ...
          console.log( typeof array[e].title)
          if((buscar != "")
          && (array[e].title.toLowerCase().includes(buscar.toLowerCase()))){
            filtradas.push(array[e])}}
            for(i in filtradas){
      
           console.log(filtradas[i].title)
      
      
            htmlContentToAppend +=
            
            `
            <div class="offcanvas offcanvas-top" tabindex="-1" id="target${filtradas[i].id}" aria-labelledby="offcanvasExampleLabel"">
            <div class="offcanvas-header">
              <h5 class="offcanvas-title" >${filtradas[i].title}</h5>
              <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            <div class="offcanvas-body">
              <div>
                ${filtradas[i].overview}
              </div><hr>
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <li> Year :${filtradas[i].release_date}</li>
                <li> Runtime :${filtradas[i].runtime} mins</li>
                <li> Revenue:  $ ${filtradas[i].release_date}</li>
                <li>Budget: $ ${filtradas[i].budget}</li>
      ...
      <div  class="card" class="card" "${filtradas[i].title}">
      <div class="card-title">
      <h4  style="margin-left:20px; display:inline; float:left">${filtradas[i].title} `+  
      ` <h4 style="margin-right:20px; margin-right:40px; display:inline; float:right">${stars(filtradas[i].vote_average/2)}${starsBlack(filtradas[i].vote_average/2)}</h5> 
      <br>
      <p class="card-body">${filtradas[i].tagline} </div> 
      <footer>
      
      <button class="btn btn-primary" type="button" data-bs-toggle="offcanvas" data-bs-target="#target${filtradas[i].id}" aria-controls="offcanvasExampleLabel">
      Mas info
      </button>
      </footer></div> </div> 
